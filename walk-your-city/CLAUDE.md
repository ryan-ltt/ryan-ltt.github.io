...
- Sign-in modal (fixed overlay) supporting both sign-in and sign-up flows.
...
- `migrateLocalStorageIfNeeded()` — one-time migration on sign-in: bulk-upserts all three cities' localStorage data to Supabase in 500-row batches, then marks completion in localStorage.
...
City OSM relations: Toronto (324211), Vancouver (1852574), Montréal (1634158). Waits 5 s between cities to be polite to Overpass.

### `data/toronto.json`, `vancouver.json`, `montreal.json`
...
- **Auth is opt-in.** The app is fully functional with only `localStorage`. Supabase enables cross-device sync.
